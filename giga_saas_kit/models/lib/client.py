...
nginx_vhost = "/var/lib/giga/giga-SAAS-Data/docker_vhosts/"
data_dir = "/var/lib/giga/giga-SAAS-Data/"
...
def delete_remote_data_dir(domain,ssh_obj):
    ...
    if "giga-server.conf" in sftp.listdir(path):#os.path.exists(path+"/giga-server.conf"):
        try:
            if "13.0" in sftp.listdir(path+"/data-dir/addons/"):#os.path.exists(path+"/data-dir/addons/12.0"): 
                _logger.info("Permissions of giga addons/13.0")
...
def delete_data_dir(domain):
    path = '{}/{}'.format(data_dir,domain)
    _logger.info("$$$$$ %r "%path)
    if os.path.exists(path+"/giga-server.conf"):
        try:
...
def update_values(config_path):
    ...
    nginx_vhost = parser.get("options","giga_saas_data")+"/docker_vhosts"
    data_dir = parser.get("options","giga_saas_data")
