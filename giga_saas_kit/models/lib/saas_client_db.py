...
@check_error
def create_new(client,database_name,user,passwd,admin_passwd): #send giga admin password. Using default (Admin) for now
...
def create_saas_client(operation = None, giga_url=None, giga_username = None, giga_password = None, base_db=None, database_name=None,modules_list=[],admin_passwd = "admin"):
    ...
    client = connect_db(giga_url, base_db, giga_username, giga_password, flag = (True if operation != "clone" else False)) # base db for cloningelse it doesn't matter
    if not client:
        return response
...
#     
    if operation == 'clone':
        ...
        response['db_create'] = create_new(client, database_name, giga_username, giga_password,admin_passwd) # new database name along with credentials

    if len(modules_list) and operation == 'install':
       client = connect_db(giga_url,database_name, giga_username, giga_password, flag = False)
       if not client:
